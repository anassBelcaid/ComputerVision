...
def objective_func(x, **kwargs):
    ...
    #projecting into 2d
    projected = projection(P,pts3d)

    diff = projected - pts2d
...
def projection(P: np.ndarray, points_3d: np.ndarray) -> np.ndarray:
    ...
    #fix additional dimension for point_3d
    if m==3:
        points_3d = np.c_[points_3d, np.ones(n)]

    #taking the matrix product
    projected_points_2d = points_3d @ P.T

    #normalizing
    normalizing_factor = projected_points_2d[:,2]
    normalizing_factor = normalizing_factor[:,None]

    projected_points_2d = projected_points_2d/normalizing_factor

    #taking only the first components
...
def estimate_camera_matrix(pts2d: np.ndarray, 
    ...
    ##############################
    # TODO: Student code goes here
    points = {'pts2d':pts2d, "pts3d":pts3d}
    initial_guess = initial_guess.flatten()[:-1]
    M = None
    out = least_squares(objective_func, initial_guess, method='lm',
                        verbose=2, max_nfev=50000, kwargs=points)
    M = np.append(out['x'], 1).reshape(3,4)
    print(M.shape)
    ##############################
