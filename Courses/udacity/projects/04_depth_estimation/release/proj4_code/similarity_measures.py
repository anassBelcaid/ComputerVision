...
def ssd_similarity_measure(patch1: torch.Tensor, patch2: torch.Tensor) -> torch.Tensor:
  ...
  """
  assert patch1.shape == patch2.shape, "patches not the same size on ssd"
  ssd_value = 0 #placeholder
  ############################################################################
  # Student code begin
  ############################################################################
  ssd_value = torch.norm(patch1- patch2)**2
...
def sad_similarity_measure(patch1: torch.Tensor, patch2: torch.Tensor) -> torch.Tensor:
  ...
  assert patch1.shape == patch2.shape, "patches not the same size on sad"
  sad_value = 0 #placeholder
  ############################################################################
  # Student code begin
  ############################################################################

  return torch.norm(patch1 - patch2, p =1)
