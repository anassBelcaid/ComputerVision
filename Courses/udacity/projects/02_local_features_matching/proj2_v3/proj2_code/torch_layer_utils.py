...
def get_gaussian_kernel(ksize=7, sigma=5) -> torch.nn.Parameter:
    ...
    ### TODO: YOUR CODE HERE ###
    mean = ksize //2
    kernel = np.arange(ksize) - mean
    const = 1/(np.sqrt(2* np.pi)*sigma)

    kernel = const * np.exp( - (kernel)**2 /(2*sigma**2))
    kernel = np.outer(kernel, kernel)
    kernel /= kernel.sum()
    kernel = np.tile(kernel, (3,1, 1, 1))

    kernel = torch.nn.Parameter(torch.Tensor(kernel))
...
def get_sobel_xy_parameters() -> torch.nn.Parameter:
    ...
    kernel = torch.zeros((2,3,3))
    #gradient in x
    kernel[0] = torch.Tensor([[-1, 0,  1], [-2, 0, 2], [-1, 0, 1]])
