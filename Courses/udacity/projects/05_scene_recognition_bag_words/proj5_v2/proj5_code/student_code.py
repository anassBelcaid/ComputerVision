...
def pairwise_distances(X, Y):
    ...
    # TODO: YOUR CODE HERE                                                    #
    ###########################################################################
    D = np.linalg.norm( X[:,:,None] - Y.T[None,:,:], axis=1)
    return D
...
def get_tiny_images(image_arrays):
    ...
    feats = np.zeros((len(image_arrays), 256))

    ###########################################################################
    # TODO: YOUR CODE HERE                                                    #
    ###########################################################################
    for i,image in enumerate(image_arrays):
        feats[i] = cv2.resize(image, (16,16)).flatten()
...
def nearest_neighbor_classify(train_image_feats, train_labels,
    ...
    for i in range(n):
        ...
        nei_labels = np.array([train_labels[n] for n in neighbors]).tolist()

        #bincounts
        counts = [nei_labels.count(k) for k in nei_labels]
        most = nei_labels[np.argmax(counts)]
        test_labels.append(most)
...
def kmeans(feature_vectors, k, max_iter = 100):
    ...
    # TODO: YOUR CODE HERE                                                      #
    #############################################################################
    #1 sample without repitition from the features
    success = False
    #avoid a random sample that give a zero class
    while not success:
        indices = np.random.choice(np.arange(n), k, replace=False)
        centroids = feature_vectors[indices]

        #compute labels
        labels = pairwise_distances(feature_vectors, centroids).argmin(axis=1)

        #count all class
        counts = np.array([labels.tolist().count(j) for j in range(k)])
        if np.alltrue(counts>0):
            success= True
    ...
    while not success and iter < max_iter:
        ...
        for j in range(k):
            mask = (labels == j)
            centroids[j] = feature_vectors[mask].mean(axis=0)


        #compute new labels
        new_labels = pairwise_distances(feature_vectors, centroids).argmin(axis=1)

        if np.alltrue(labels == new_labels):
            success = True
...
def build_vocabulary(image_arrays, vocab_size, stride = 20):
    ...
    ###########################################################################
    features = []
    for sample in image_arrays:
        n, m = sample.shape
        #converting
        sample = torch.tensor(np.array(sample, dtype='float32')).reshape(1,1,*sample.shape)

        #point extraction
        X, Y = np.arange(10,n-10,stride), np.arange(10, m-10, stride)
        X, Y = np.meshgrid(X,Y)

        #getting features
        feat  = get_siftnet_features(sample, Y.flatten(), X.flatten())
        features.extend(feat.tolist())

    #now gettting the vocabulary by k means
